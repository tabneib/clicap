...
public abstract class AbstractService extends IcapParser {
	...
	public int fullResponse(ByteArrayOutputStream bas) throws Exception {
		...
		// @NHD ICAP message format, see: https://tools.ietf.org/html/rfc3507	
		...
		switch (this.type){
			case REQMOD:
				...
				sb.append(this.resHeader.length()).append(CRLF); 
	...
	public int earlyResponse(ByteArrayOutputStream bas)  throws Exception{
		...
		// @NHD
		//System.out.println("earlyResponse called !!!");
	...
	public synchronized TYPE parse(DataInputStream bufferedreader) throws Exception {
		...
		try {
			switch (this.type){
				case REQMOD: 
					// @NHD
					// System.out.println("parsing HTTP message in REQMOD");
	...
	public void parseHttpRequest(DataInputStream bufferedreader, StringBuilder requestHeader) throws Exception {
		...
		// @NHD Why is content not parsed ?
		// Let's read it :)
		// TODO The content is encoded, find a way to decode it !
		/*if( getReqHeader("content-length") != null && Integer.parseInt(getReqHeader("content-length"))>0){
			
			System.out.println("\n\nTRYING TO PARSE BODY IN REQMODE:\n");
			int bodyLength = 11;
			byte[] body = new byte[bodyLength];
			int mustread = bodyLength;
			while (mustread>0){
				mustread-=bufferedreader.read(body, bodyLength-mustread, mustread);
			}
