...
public class IcapServer extends Thread implements Icap {
	...
	public IcapServer(String name, String IP, int port, String serviceconfigfile){
		...
		if (ka.equals("off") || ka.startsWith("disable") || ka.equals("no") || ka.equals("close") ){
			...
			// @NHD remove output message to make terminal clearer
...
//	<--------------------------------------------------------------------------->  
	...
	private static void readConf(String config) {
		...
		try {       
			...
			// @NHD remove outpur message to make terminal clearer
			...
			while ((str = in.readLine()) !=null ){
				...
				if (values[0].equalsIgnoreCase("icap")){
					...
					// @NHD remove this output text to make the terminal clearer
...
//	<--------------------------------------------------------------------------->
	...
	public void enable(){
		...
		// @NHD Change to a more reasonable message
		String message = "\nICAP Server at server " + Clicap.config.get(Clicap.ID) + " is started.\n";
		...
		// @NHD Change to a more reasonable message
		message = "ICAP Server at server " + Clicap.ID + " is started.";
		//if (!turnStdOff) System.out.println(message);
		Log.error(Log.INFO, message);
	}
//	<--------------------------------------------------------------------------->
